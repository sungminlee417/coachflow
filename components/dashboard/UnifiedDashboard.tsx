...
const sectionTone: Record<
  Section,
  { active: string; activeIcon: string; mobileActive: string }
> = {
  coaching: {
...
export default function UnifiedDashboard({
  ...
  const fetchCoach = async () => {
    try {
      ...
      if (relationship?.profiles)
        setCoach(relationship.profiles as unknown as Profile);
    } catch {
    } finally {
  ...
  const renderNavButton = (tab: TabDef) => {
    ...
    return (
      ...
      >
        <span className={isActive ? tone.activeIcon : "text-slate-400"}>
          {tab.icon}
        </span>
        {tab.label}
      </button>
          ...
          <nav className="flex-1 px-3 py-6 space-y-6 overflow-y-auto">
            <div>
              ...
              <div className="space-y-1">
                {coachingTabs.map((t) => renderNavButton(t))}
              </div>
            </div>
            <div>
              <p className="px-3 text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">
              ...
              <div className="space-y-1">
                {trainingTabs.map((t) => renderNavButton(t))}
              </div>
            </div>
          </nav>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          You can assign workouts and meal plans to yourself, or
                          use an invite link to connect with a coach.
                        </p>
                      </div>
                    </div>
                  )}
