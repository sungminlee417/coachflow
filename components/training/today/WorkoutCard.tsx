        ...
        <div className="space-y-2.5">
          ...
          {summary?.nextSet ? (
            ...
            <p className="text-xs text-muted truncate">
              Next:{' '}
              <span className="font-medium text-foreground">{summary.nextExercise}</span>
              <span className="ml-1 text-subtle">— open to log</span>
            </p>
          ) : null}
          {summary && summary.totalSets > 0 && summary.completedSets >= summary.totalSets && (
            <p className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-fg bg-emerald-soft border border-emerald-line rounded-full px-2 py-0.5">
      ...
      <p className="text-[11px] text-muted truncate">
        ...
        <span className="text-subtle">
          ...
          {!isCardio && targetReps && (
            <>
              {' · '}target{' '}
              <span className="font-medium text-muted">{targetReps}</span>
            </>
          )}
