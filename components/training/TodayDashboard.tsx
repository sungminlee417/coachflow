...
function WorkoutCard({
  ...
  const summary = useMemo(() => {
    ...
    // The "next set you should log" — limited to plain strength
    // exercises (no superset, no cardio) because the inline form only
    // takes weight + reps. Anything else routes to the deep view.
    let nextSet: {
      assignmentId: string
      exerciseId: string
      exerciseName: string
    ...
    for (const a of assignments) {
      ...
      for (let i = 0; i < exercises.length; i++) {
        ...
        // Skip cardio and supersets (any pair_with_next chain) — those
        // need richer UI than weight + reps.
        ...
        if (
          ...
          const targetRepsStr =
            ex.exercise_sets?.[target - 1]?.target_reps ?? ex.reps ?? ''
          nextSet = {
            assignmentId: a.id,
            exerciseId: ex.id,
...
function NextSetMiniLogger({
  ...
  return (
    <div className="rounded-xl bg-emerald-50/40 border border-emerald-100 p-2.5 mt-1">
