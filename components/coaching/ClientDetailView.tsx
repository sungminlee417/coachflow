...
export default function ClientDetailView({
  ...
  >([]);
  const [mealPlanAssignments, setMealPlanAssignments] = useState<
    MealPlanAssignmentRow[]
  >([]);
  const [workouts, setWorkouts] = useState<WorkoutOption[]>([]);
  const [mealPlans, setMealPlans] = useState<MealPlanOption[]>([]);
  const [loading, setLoading] = useState(true);
  const [assigningWorkout, setAssigningWorkout] =
    useState<WorkoutOption | null>(null);
  const [assigningMealPlan, setAssigningMealPlan] =
    useState<MealPlanOption | null>(null);
  ...
  return (
    <div>
      ...
      <ChatBox
        ...
        client={client}
        open={directMessage}
        onClose={() => setDirectMessage(false)}
      ></ChatBox>
      <ConfirmDialog
        open={!!pendingDelete}
        title={
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            font-medium transition-colors cursor-pointer focus:outline-none focus:ring-2 
          ...
          <select
            ...
            defaultValue=""
            className="px-3 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm 
            font-medium transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500"
          ...
          <select
            ...
            text-sm font-medium transition-colors cursor-pointer focus:outline-none focus:ring-2 
      ...
      {mealPlanAssignments.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-6 text-center mb-8">
          ...
          <p className="text-xs text-slate-400">
            Pick one from the &ldquo;Assign Meal Plan&rdquo; menu above
          </p>
        </div>
      ) : (
          ...
          {mealPlanAssignments.map((a) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <div
                  className="h-9 w-9 rounded-lg bg-emerald-50 flex items-center 
