  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button
        ...
        type="button"
        onClick={() => setOpen(o => !o)}
        className={`w-full flex items-center gap-2 px-3 py-2.5 border border-line rounded-lg text-sm bg-surface hover:border-subtle focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors cursor-pointer ${
 value ? 'text-foreground ' : 'text-subtle '
 }`}
      >
        <Calendar size={16} className="text-subtle shrink-0" />
      ...
      {open && (
        <div
          className="absolute z-50 mt-2 bg-surface rounded-xl border border-line shadow-xl p-3 left-0"
          style={
            {
